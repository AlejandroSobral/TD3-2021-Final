...
__attribute__(( section(".functions"))) void Load_KeyBoardTable(Table_Buffer* table_p, Keyboard_buffer* buff_p)
{
    ...
    if (table_index > DIGIT_TABLE_SIZE)
	{
...
//This function were temporary. I said were.
...
{	bits16_v offset = 0;
	...
	bits32_v aux_H = 0;
	byte Aux_var = 0;
    byte length_2, length_t1, length_t2, length_t3;    
	byte string[] = "Alejandro Sobral - UTN FRBA - TDIII - 2021.";
	byte string2[] = "- Intel x86 -";    
    byte string_TASK1[] = "Promedio TAREA 1: ";
    byte string_TASK2[] = "Suma     TAREA 2: ";
    byte string_TASK3[] = "Suma     TAREA 3: ";    
    byte nmbr[16];
	bits16_v length = 0;
	length = sizeof(string);
	length_2 = sizeof(string2);    
    length_t1 = sizeof(string_TASK1);
    length_t2 = sizeof(string_TASK2);
    length_t3 = sizeof(string_TASK3);        
	...
	for(offset = 0; offset < length; offset++)
	{
		...
		Video_Buff_v->buffer[((0 + offset) * 2 + 1) + (24 * 160)] = 0x43;	//Set attrb.
    }
	for(offset = 0; offset < length_2; offset++)
	{
		Video_Buff_v->buffer[((10 + offset) * 2) + (3 * 160)] = string2[offset];	//Set Data
		Video_Buff_v->buffer[((10 + offset) * 2 + 1) + (3 * 160)] = 0x43;	//Set attrb.
    }    

    	for(offset = 0; offset < length_t1; offset++)
	{
		Video_Buff_v->buffer[((column -(length_t1 ) + offset) * 2) + (0 * 160)] = string_TASK1[offset];	//Set Data
		Video_Buff_v->buffer[((column -(length_t1 ) + offset) * 2 + 1) + (0 * 160)] = 0x09;	//Set attrb.
    }
    	for(offset = 0; offset < length_t2; offset++)
	{
		Video_Buff_v->buffer[((column -(length_t2 ) + offset) * 2) + (1 * 160)] = string_TASK2[offset];	//Set Data
		Video_Buff_v->buffer[((column -(length_t2 ) + offset) * 2 + 1) + (1 * 160)] = 0x09;	//Set attrb.
    }
    	for(offset = 0; offset < length_t3; offset++)
	{
		Video_Buff_v->buffer[((column -(length_t3 ) + offset) * 2) + (2 * 160)] = string_TASK3[offset];	//Set Data
		Video_Buff_v->buffer[((column -(length_t3 ) + offset) * 2 + 1) + (2 * 160)] = 0x09;	//Set attrb.
    }
}    
...
__attribute__(( section(".functions"))) byte buffer_Pop(Keyboard_buffer* key_buffer) 
{
	...
	if(key_buffer->Loaded_elements > 0)	
	{
		...
		if(key_buffer->out_index == KEYBOARD_BUFFER_SIZE)
		{
...
__attribute__(( section(".functions"))) void __Keyboard_Buffer_func( Keyboard_buffer* key_buffer, byte value_to_store){ 
    ...
    if(key_buffer->in_index >= KEYBOARD_BUFFER_SIZE)
    ...
    if( key_buffer->Loaded_elements== KEYBOARD_BUFFER_SIZE)
    {
        key_buffer->Loaded_elements = KEYBOARD_BUFFER_SIZE; //Prevent any type of overflow from now on
    }
